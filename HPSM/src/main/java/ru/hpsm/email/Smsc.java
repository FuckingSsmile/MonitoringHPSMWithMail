...
public class Smsc implements Runnable{
    ...
    public Smsc(String login, String password, LinkedList<SmsPackage> smsPackages, Set<String> phones) {
        SMSC_LOGIN = login;
    ...
    public void run() {
        while (true){
            try {
                Thread.sleep(settings.getInteger(Settings.KeysInteger.SMSC_ANTISPAM_SEC)*1000);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }

            if(smsPackages != null && !smsPackages.isEmpty()){
                sendSms(smsPackages.pollFirst());
            }
        }
    }

    private void sendSms(SmsPackage smsPackage) {
        String phones = this.allphones.toString();
